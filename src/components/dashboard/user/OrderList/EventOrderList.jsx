...
const EventOrderList = () => {
  ...
  const { data: invoice = [], refetch, isLoading } = useQuery({
    queryKey: ["users"],
    queryFn: () =>
      fetch(`process.env.NEXT_PUBLIC_SERVER_SIDE_BASE_URL/myAllOrder/${session?.data?.user?.email}`).then((res) =>
        res.json()
      ),
  });
  console.log(invoice);
  ...
  const handleRefundRequest = async (id) => {
    const res = await axios.put(`process.env.NEXT_PUBLIC_SERVER_SIDE_BASE_URL/refundRequest/${id}`)
    console.log(res)
