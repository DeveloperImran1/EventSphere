...
const Cards = () => {
  ...
  const { data: myPosts } = useQuery({
    queryKey: ["myPost"],
    queryFn: () =>
      fetch(`process.env.NEXT_PUBLIC_SERVER_SIDE_BASE_URL/getUserPosts/${lastPathSegment}`).then((res) =>
        res.json()
      ),
  });
  ...
  const handleFollow = async () => {
    ...
    try {
      const result = await axios.put("process.env.NEXT_PUBLIC_SERVER_SIDE_BASE_URL/userAddedFollower", addedFollower);
      console.log(result);
